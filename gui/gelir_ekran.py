...
def gelir_ekrani(parent, finans):
    pencere = tk.Toplevel(parent)
    pencere.title("Gelir Ekle")
    pencere.geometry("350x350")
    pencere.grab_set()

    tk.Label(pencere, text="Gelir Ekle", font=("Arial", 14)).pack(pady=10)

    tk.Label(pencere, text="Gelir Adı").pack()
    ...
    def kaydet():
        ...
        if not ad or not kategori:
            messagebox.showerror("Hata", "Tüm alanları doldur hocam")
            return
